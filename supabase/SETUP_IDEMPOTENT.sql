-- IDEMPOTENT: Anonymous Chat Setup (Safe to run multiple times)
-- This script can be run repeatedly without errors

-- ============================================================
-- PART 1: ENSURE TABLES EXIST (with IF NOT EXISTS)
-- ============================================================
...
-- ============================================================
-- PART 2: CREATE INDEXES (safe to run multiple times)
-- ============================================================
...
ALTER TABLE IF EXISTS public.anonymous_chat_sessions ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS public.anonymous_chat_messages ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS public.anonymous_chat_archive ENABLE ROW LEVEL SECURITY;

-- ============================================================
-- PART 4: DROP OLD POLICIES (to recreate them cleanly)
-- ============================================================

DROP POLICY IF EXISTS "view_own_or_waiting_sessions" ON anonymous_chat_sessions;
DROP POLICY IF EXISTS "create_waiting_session" ON anonymous_chat_sessions;
DROP POLICY IF EXISTS "update_own_sessions" ON anonymous_chat_sessions;
DROP POLICY IF EXISTS "view_messages_in_session" ON anonymous_chat_messages;
DROP POLICY IF EXISTS "send_messages_in_paired_session" ON anonymous_chat_messages;
DROP POLICY IF EXISTS "delete_own_messages" ON anonymous_chat_messages;
DROP POLICY IF EXISTS "view_own_archives" ON anonymous_chat_archive;

-- Remove old policy names too (in case they exist)
...
-- ============================================================
-- PART 5: CREATE NEW RLS POLICIES (fresh)
-- ============================================================
...
-- ============================================================
-- PART 6: CREATE OR REPLACE FUNCTION (safe on repeated runs)
-- ============================================================
...
-- Expected output:
-- anonymous_chat_archive       | 1 policy
-- anonymous_chat_messages      | 3 policies
-- anonymous_chat_sessions      | 3 policies

-- ============================================================
-- SUCCESS: All tables, indexes, policies, and functions ready!
